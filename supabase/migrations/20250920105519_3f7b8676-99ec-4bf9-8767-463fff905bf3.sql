...
-- Note: inspector_basic_info is a view, so we cannot apply RLS to it directly
-- The security must be implemented in the underlying tables that the view queries

-- 1. Strengthen inspector_contacts policies
...
-- Only allow viewing if user is admin OR has specific elevated access for that specific inspector
...
USING (
  ...
  AND (
    ...
    OR
    -- Non-admin users can only view contacts for inspectors they have explicit elevated access to
    EXISTS (
      SELECT 1
...
-- 2. Update the get_inspector_contact_secure function to be more restrictive and add proper audit logging
