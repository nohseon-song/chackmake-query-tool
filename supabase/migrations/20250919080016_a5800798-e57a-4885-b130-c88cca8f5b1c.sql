...
-- 4. Add audit logging for contact access
-- Create a function to log contact access attempts
CREATE OR REPLACE FUNCTION public.log_inspector_contact_access()
RETURNS TRIGGER AS $$
BEGIN
  ...
  INSERT INTO public.audit_logs (
    ...
    jsonb_build_object(
      ...
      'accessed_at', now(),
      'user_role', (SELECT role FROM public.user_profiles WHERE id = auth.uid())
    )
  );
...
DROP TRIGGER IF EXISTS audit_inspector_contact_access ON public.inspector_contacts;
CREATE TRIGGER audit_inspector_contact_access
  AFTER SELECT ON public.inspector_contacts
  FOR EACH ROW
  EXECUTE FUNCTION public.log_inspector_contact_access();

-- Note: PostgreSQL doesn't support AFTER SELECT triggers, so we'll use a different approach
-- Remove the trigger and rely on application-level logging instead
DROP TRIGGER IF EXISTS audit_inspector_contact_access ON public.inspector_contacts;
DROP FUNCTION IF EXISTS public.log_inspector_contact_access();

-- 5. Ensure security definer functions are properly secured
-- Review and update the existing security definer functions to ensure they're not bypassing RLS

-- Update the get_inspector_contact_secure function to be more restrictive
